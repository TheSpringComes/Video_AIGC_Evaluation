...
flags.DEFINE_string('ckpt_path', './/weights//musiq_koniq_ckpt.npz', 'Path to checkpoint.')
flags.DEFINE_string('video_dir_path', '..//vids//out_imgs', 'Path to input image.')
...
def run_model_multi_images(model, num_classes, pp_config, params, images_dir_path):
  """Runs the model.

    Args:
    pp_config: image preprocessing config.
    params: model parameters loaded from checkpoint.
    video_dir_path: input image path.
...
def main(_):
  ...
  model = model_mod.Model.partial(num_classes=FLAGS.num_classes, train=False, **model_config)

  avg_pred_mos = []
  images_dir_path = os.listdir(FLAGS.video_dir_path)
  for image_path in tqdm(images_dir_path, desc='Predicting videos'):
    vid_pred_mos = run_model_multi_images(model, 
                                          FLAGS.num_classes,
                                          pp_config, 
                                          params, 
                                          os.path.join(FLAGS.video_dir_path, image_path))
    avg_pred_mos.append(vid_pred_mos)
  print('============== Average Precited MOS:', np.mean(avg_pred_mos), '==============')
